...
class Material(Base):
    ...
    def add_material(cls, name):
        ...
        msg = cursor.var(cx_Oracle.STRING)
        cnt = cursor.var(cx_Oracle.NUMBER)
        cursor.callproc("materials_p.add_material", [name,cnt,msg])
        connection.commit()
        return msg.getvalue()
...
class Vendor_Material(Base):
    ...
    @classmethod
    def add_vend_mat(cls,mat_id_fk,v_ed_fk):

        cnt = cursor.var(cx_Oracle.NUMBER)
        msg = cursor.var(cx_Oracle.STRING)

        cursor.callproc("helper_tables_p.add_vend_mat", [mat_id_fk,v_ed_fk,cnt,msg])
        connection.commit()

        return msg.getvalue()

    @classmethod
    def update_vend_mat(cls,new_mat_id_fk,v_ed_fk):

        cnt = cursor.var(cx_Oracle.NUMBER)
        msg = cursor.var(cx_Oracle.STRING)

        cursor.callproc("helper_tables_p.update_vend_mat", [new_mat_id_fk,v_ed_fk,cnt,msg])
        connection.commit()

        return msg.getvalue()

    @classmethod
    def delete_vend_mat(cls, v_ed_fk):
        msg = cursor.var(cx_Oracle.STRING)

        cursor.callproc("helper_tables_p.delete_vend_mat", [v_ed_fk,msg])
        connection.commit()
