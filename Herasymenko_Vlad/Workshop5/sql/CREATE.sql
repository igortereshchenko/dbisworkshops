...
v_ed_fk NUMBER(8) NOT NULL, /*FK*/
mat_id_fk number(8) NOT NULL, /*FK*/
price number(10) NOT NULL,
record_month Number(2,0) NOT NULL, /*PK*/
record_year Number(4,0) NOT NULL /*PK*/
);

/*Data updates every first day of the month*/
...
CREATE TABLE BANK_T( /* Допоміжна таблиця, для збереження інформації про банк постачальника */
...
CREATE TABLE VEND_MAT_T( /*Допоміжна таблиця між вендорами та матеріалами*/
...
ADD CONSTRAINT history_pk PRIMARY KEY(v_ed_fk,mat_id_fk,record_month,record_year)
/
ALTER TABLE Prices_History
ADD CONSTRAINT m_id_fk_hist FOREIGN KEY (mat_id_fk,v_ed_fk) REFERENCES VEND_MAT_T(MATERIAL_ID_FK,VEND_EDRPOU_FK);
